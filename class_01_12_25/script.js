...
function getWeather(city) {
    ...
    fetch(url).then((response) => response.json()).then((data) => {
        ...
        document.getElementById("sunset").innerHTML = ` Восход: ${new Date(data.sys.sunset).getHours()}: ${new Date(data.sys.sunset).getMinutes()}: ${new Date(data.sys.sunset).getSeconds()}`;
